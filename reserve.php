...
<head>
    ...
    <title>Reserve - CIT ProJector Reservation</title>
        ...
        <div class="container">
            <div class="row">
                ...
                <div class="h1-text col-lg-12 text-center">
                    <h1 style="margin-top: -10px; margin-bottom: 30px;"><span style="color: #471847; font-weight: bolder;">CIT Pro</span><span style="color: #FECC6B; font-weight: bolder; font-size: 45px;">J</span><span style="color: #471847; font-weight: bolder;">ector</span> | Reserve</h1>
...
<script>
   ...
   <div class="popUp" id="errornotifPopup">
    ...
    <script>
        ...
        // JavaScript function to close success message pop-up
...
function deleteUnusedProjectors($conn) {
    $delete_sql = "DELETE FROM projector WHERE proj_id NOT IN (SELECT DISTINCT proj_id FROM reservation)";
    if ($conn->query($delete_sql) === TRUE) {
        // projectors successfully deleted
    } else {
        echo "Error deleting unused projectors: " . $conn->error;
    }
}

if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['reserve-btn'])) { 

    if (isset($_SESSION['u_id'])) {
        $room = $_POST['Room'];
        $date = $_POST['Date'];

        // convert Start and End Time to 24-hour format for database comparison
        $start_time = date("H:i:s", strtotime($_POST['StartTime']));
        $end_time = date("H:i:s", strtotime($_POST['EndTime']));
        $user_id = $_SESSION['u_id'];
        $proj_id = $_SESSION['p_id'];
    
        // check if the room is already reserved for the selected time slot
        $room_check_sql = "SELECT `room` FROM `reservation` WHERE `room` = ? AND `date` = ? AND (
            (`start_time` <= ? AND `end_time` > ?) OR
            (`start_time` < ? AND `end_time` >= ?)
        )";
        $room_check_stmt = $conn->prepare($room_check_sql);
        $room_check_stmt->bind_param("ssssss", $room, $date, $start_time, $start_time, $end_time, $end_time);
        $room_check_stmt->execute();
        $room_check_result = $room_check_stmt->get_result();
    
        if ($room_check_result->num_rows > 0) {
            $errorMessage = "$room is occupied on $date from " . date("h:i A", strtotime($start_time)) . " to " . date("h:i A", strtotime($end_time)) . ".";
            echo "<script>showErrorMessage('$errorMessage');</script>";
        } else {
            // check if the projector is already reserved for the selected time slot
            $check_sql = "SELECT `proj_id` FROM `reservation` WHERE `proj_id` = ? AND `date` = ? AND (
                (`start_time` <= ? AND `end_time` > ?) OR
                (`start_time` < ? AND `end_time` >= ?)
            )";
            $check_stmt = $conn->prepare($check_sql);
            $check_stmt->bind_param("isssss", $proj_id, $date, $start_time, $start_time, $end_time, $end_time);
            $check_stmt->execute();
            $check_result = $check_stmt->get_result();
    
            if ($check_result->num_rows > 0) {
                // reservation already exists for the projector
                $errorMessage = $_SESSION['p_name'] . " is already reserved on $date at " . date("h:i A", strtotime($start_time)) . " to " . date("h:i A", strtotime($end_time)) . ".";
                echo "<script>showErrorMessage('$errorMessage');</script>";
            } else {
                // no existing reservation for the projector or room
                $insert_sql = "INSERT INTO `reservation`(`user_id`, `proj_id`, `room`, `date`, `start_time`, `end_time`) VALUES (?, ?, ?, ?, ?, ?)";
                $insert_stmt = $conn->prepare($insert_sql);
                $insert_stmt->bind_param("iissss", $user_id, $proj_id, $room, $date, $start_time, $end_time);
    
                if ($insert_stmt->execute()) {
                    // reservation successful
                    $successMessage = $_SESSION['p_name'] . " Reserved Successfully!";
                    echo "<script>
                    ...
                    $errorMessage = $_SESSION['p_name'] . " Reservation Unsuccessful! " . $insert_stmt->error;
                    echo "<script>showErrorMessage('$errorMessage');</script>";
                }
    ...
    // delete unused projectors after making a reservation
    deleteUnusedProjectors($conn);
}
